					...
					if ($sirvClient->testConnection()) {  
					  ...
					  // echo "connected";
						$res = $sirvClient->uploadFile(
						  ...
						  $destFile,
						  // Local file name
						 $_FILES["image_pic"]["tmp_name"] 
						);
						if($old_image)
							{
								$old_image_path="about_images/".$old_image;  
								$sirvClient->deleteFile($old_image_path);  
							}
						if($res['full_url']=='')
						{
							$destFile = "/home/koofamilies/public_html/images/about_images/".$uniquesavename;
							move_uploaded_file($_FILES["image_pic"]["tmp_name"],$destFile); 
						}
						else
						{
							$image_cdn_url=$res['full_url'];
						}
...
$path= '/home/koofamilies/public_html/images/videos';
...
{ 
if (move_uploaded_file($_FILES['file']['tmp_name'], "/home/koofamilies/public_html/images/videos/" . $_FILES['file']['name'])) {
 //echo 'Uploaded!';
}
}
}
 ...
 $name_video = $name != "" ? $name  : $_POST['video_up'];
	...
	{
		   $id_abt = $_POST['id']; 
				 $test_video =  mysqli_query($conn, "UPDATE about SET description='$description',link='$link', welcome_note='$welcome_note',image='$uniquesavename',video_upload='$name_video ' WHERE userid='$id_abt'");
 header("Location: ".$site_url."/about_us.php");   
  ...
  else
  {
	  ...
	  $insert_test  =  mysqli_query($conn, "INSERT INTO about SET userid='$id_abt',description='$description',link='$link',welcome_note='$welcome_note',image='$uniquesavename',video_upload='$name_video'");
