...
if($s_id)
{
	...
	if($row)
	{
						 ...
						 if($sstper>0){
							$incsst = ($sstper / 100) * $total;
								...
								$incsst=ceiling($incsst,0.05);
								  $incsst=@number_format($incsst, 2);
							     $g_total=@number_format($total+$incsst+$row['speed_delivery_amount'], 2);
								} else { $g_total=$total + $row['speed_delivery_amount'];} 
		...
		if($v_comisssion)
		{
			...
			$msg_str.="<br/>Cash term Pay:(".$total."+(chinese man delivery) ".$speed_delivery_label.")-".$v_comisssion."(com)=".number_format($total-$v_comisssion,2);
			else
			$msg_str.="<br/>Cash term Pay:".$total.$speed_delivery_label."-".$v_comisssion."(com)=".number_format($total-$v_comisssion,2);
		}
		else
		{
			if($special_delivery_amount)
			$msg_str.="<br/>Cash term Pay:".number_format($total,2)."+".number_format($special_delivery_amount,2)."(chinese man delivery)".$speed_delivery_label;
			else
			$msg_str.="<br/>Cash term Pay:".number_format($total,2).$speed_delivery_label;	  
		}
