...
#remarks_area .modal-body{
 ...
 img.lazy {
        ...
        background-image: url('https://koofamilies.com/img/logo.png');
  ...
  <div style="float:left;width:100%;top:0;display:none;" id="sec_table_show">   
    ...
    <div>
      ...
      <div class="input-group mb-2 mobile_no_label"  style="margin-bottom:0px !important;">
        ...
        <input type="number" autocomplete="tel" maxlength='10' id="mobile_number" style="max-width:220px;"  class="mobile_number form-control" value="<?php if($check_number){ echo $check_number;}  ?>" placeholder="<?php echo $language['key_in_phone']; ?>" name="mobile_number" required="" />
  ...
  <input type="hidden" id="deliver_tax_amount" name="deliver_tax_amount" value="0">
			...
			`		</div> 
					<div class="" style="display:none;" id="special_delivery_label">
					   <div style="grid-template-columns:.2fr 2fr;grid-column-gap: 10px;vertical-align: middle;align-content:center;font-weight: bold;font-size: 15px;margin-top:1%;">
						  <?php $chinese_man=$language['chinese_delivery'];echo ucfirst(strtolower($chinese_man)); ?>: Rm <span id="special_delivery_label_text"></span>
					   </div>
					</div>
					...
					<div  style="display:none;color:Red;" class="final_amount_label">
					   <div style="grid-template-columns:.2fr 2fr;grid-column-gap: 10px;vertical-align: middle;align-content:center;font-weight: bold;font-size: 15px;">
						  <?php  $payable_amount=$language['payable_amount']; echo ucfirst(strtolower($payable_amount)); ?>: Rm <span class="final_amount_value"><?php echo number_format($merchant_detail['order_extra_charge'],2); ?></span>
					   </div>
					</div>
				</div>
				<div  style="width:15%">
    ...
    <div class="row">
		...
		<input type="hidden" value='<?php echo $login_user_id;?>' name="login_for_wallet_id" id="login_for_wallet_id"/>
		   ...
		   <input type="submit" style="width:100% !important;box-shadow: -3px 3px #35cbab, -2px 2px orange, -1px 1px orange;border: 1px solid #35cbab;" class="btn btn-block btn-primary submit_button" name="cashpayment"  id="confm"  value="<?php echo $language["confirm_order"];?>">
		...
		<?php if($discounted_product || $special_coin_name){ ?>
            ...
            <input type="submit"  style="width:100% !important;box-shadow: -3px 3px #35cbab, -2px 2px orange, -1px 1px orange;border: 1px solid #35cbab;" class="btn btn-block btn-primary submit_button online_pay" name="cashpayment" 

			value='<?php if($login_user_id){  echo $language['confirm_wallet_with_login'];} else {echo $language['confirm_wallet'];} if($_SESSION['login']){ if($special_coin_name){ echo "(".$special_coin_name." $".number_format($special_bal,2).")";} else {if($user_koo_coin){ echo number_format($user_koo_coin,2);}}} ?>'/>   
							 ...
							 <div clas="form-group" style="margin-left:3%;font-size:15px;">
								...
								<?php } else { ?>
									...
									<?php echo $language['payable_amount']; ?>: Rm <span style="font-weight:bold;" class="final_amount_value"></span>
									 ...
									 Enquiry:  <a href="https://api.whatsapp.com/send?phone=60123945670" target="_blank"> 60123945670 
									 <img src="images/whatapp.png" style="max-width:32px;"/> </a>
									 </br>
			...
			<div class="modal-content">
			    ...
			    <div class="forgot-form" style="display:none;">
						  ...
						  <div class="input-group mb-2">
							...
							<input  type="number" autocomplete="tel" maxlength='12' id="forgot_mobile_number"  class="mobile_number form-control" readonly <?php if($check_number){ echo "readonly";} ?> value="<?php if($check_number){ echo $check_number;}  ?>" placeholder="Phone number" name="mobile_number" required="" />
	...
	if(number.length >= 9 && number.length <= 12 && (number[0] == 1 || number[0]==6)){
		  ...
		  else
			{
				...
				if($(this).data('type') == 'top'){
					...
					$('html, body').animate({ scrollTop: $("#mobile_no_label").offset().top }, 2000);
				  }
		   ...
		   if($('#table_type').prop('required')){
			if(table_type=='')
			{
				if($(this).data('type') == 'top'){
					//var $elem = $('#order_place');
					// $('html, body').animate({scrollTop: $elem.height()}, 1000);
					$('html, body').animate({ scrollTop: $("#table_show").offset().top }, 2000);
				  }
