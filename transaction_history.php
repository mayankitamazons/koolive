...
if(isset($_POST['search'])){
	...
	if($coin_type!="ALL")
		$filter.=" and tranfer.wallet='$coin_type'";
...
$total_rows = mysqli_num_rows(mysqli_query($conn, "(SELECT trasfer.details,tranfer.type_method,tranfer.invoice_no,tranfer.id,users.name,users.mobile_number,tranfer.amount,tranfer.wallet,tranfer.created_on,'Send' AS tx_type FROM tranfer,users WHERE tranfer.sender_id=".$_SESSION['login']." AND users.id=tranfer.receiver_id  $filter) UNION (SELECT trasfer.details,tranfer.type_method,tranfer.invoice_no,tranfer.id,users.name,users.mobile_number,tranfer.amount,tranfer.wallet,tranfer.created_on,'Receives' AS tx_type FROM tranfer,users WHERE tranfer.receiver_id=".$_SESSION['login']." AND users.id=tranfer.sender_id  $filter)"));
        ...
        <div class="content-wrapper">
            ...
            <main class="main-wrapper clearfix" style="min-height: 522px;">
                <div class="container-fluid" id="main-content" style="padding-top:25px">
					<h3><?php echo $language['tras_report']; ?></h3>  
						  ...
						  <form id="criteria_form" method="post" action=''>
							<div class="row col-sm-12">
							  ...
							  <div class="col-sm-3">
							   ...
							   <?php 
									 $walletq="select u.special_coin_name from users as u where id in(select coin_merchant_id from tranfer as t where (t.sender_id='$m_login_id' or t.receiver_id='$m_login_id'))";
									$walq=mysqli_query($conn,$walletq);
								    $wallet_count=mysqli_num_rows($walq);
									if($wallet_count>0)
									{
							  ?>
							  <div class="col-sm-3">
								...
								<label>Wallet: </label>
									<select class="form-control select2" name="coin_type">
									  <option <?php if($_POST['coin_type']=="all" || $_POST['coin_type']==""){ echo "selected";}  ?> val="all">ALL</option>
									  <?php
										while($wr=mysqli_fetch_assoc($walq))
										{
									  ?>
									  <option <?php if($_POST['coin_type']==$wr['special_coin_name']){ echo "selected";}  ?> val="<?php echo $wr['special_coin_name']; ?>"><?php echo $wr['special_coin_name']; ?></option>
										<?php } ?>
									</select>
								</div>
							  </div>
					...
					<table class="table table-striped kType_table" id="kType_table" style="width:100% !important;">
					    <thead>
					        <tr>
					    ...
					    <tbody>
							...
							?>
									...
									<td><?php echo $wat; ?></td>
									<td><?php echo $tx_history_row['remark']; ?></td>  
									<td><?php echo $tx_history_row['details']; ?></td>
