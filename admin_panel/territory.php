...
if(isset($_GET['data'])&&$_GET['data']=='deleteRecord'){
	$id = $_GET['t_id'];
	$query=mysqli_query($conn,"select * from territory where t_id='$id'");
	$c_data=mysqli_fetch_array($query);
	$category_label = $c_data['t_label'];
	
	$query_del = mysqli_query($conn,"DELETE FROM `territory` WHERE `territory`.`t_label` ='".$category_label."'");
	if($query_del){echo true;}else{die();}
}

$rec_limit = 500;
$sql_count = "select count(t_id) as total_count from territory group by t_label";
...
$query="select SQL_NO_CACHE *,GROUP_CONCAT(t_location_name SEPARATOR ', ') as t_all_name from territory  group by t_label order by t_createddate asc LIMIT $offset, $rec_limit";
        ...
        <div class="content-wrapper">
            ...
            <main class="main-wrapper clearfix" style="min-height: 522px;">
                <div class="container-fluid" id="main-content" style="padding-top:25px">
					<h3><?php echo "Territory List";?> <a href="addterritory.php" class="btn btn-primary pull-right"><?php echo "Add Territory";?></a></h3>
					<br/>
				<?php if($rec_count>25){ ?>        
					...
					<tbody>
						...
						<tr>
							<td><?php echo $i; ?></td>
							<td>
								<a href="addterritory.php?t_id=<?php echo $row['t_id'] ?>" title="Edit">
									<?php echo ucfirst($row['t_label']); ?>
								</a>	
							</td>
				
							<td><?php echo ucfirst($row['t_all_name']); ?></td>
                       	    <td>
								<a class="mr-4" href="addterritory.php?t_id=<?php echo $row['t_id'] ?>" title="Edit">
									<i class="fa fa-pencil" aria-hidden="true"></i>
								</a>
								<a href="javascript:void(0)" class="deleteRecord mr-4" t_id="<?php echo $row['t_id']?>" title="Delete">
...
<script>
	...
	$(".deleteRecord").click(function(){
		...
		var cnfrmDelete = confirm("Are You Sure Delete This Territory ?");
		if(cnfrmDelete==true){
			  $.ajax({
					url:'territory.php',
					method:'GET',
