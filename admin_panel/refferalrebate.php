...
if (empty($_GET['ms']))
{
	$url = "refferalrebate.php?ms=" . md5(rand());
	 header("Location:$url");
    exit();
}
/* end  for limit  */
if($_POST['filter']){
	...
	{
		$filter.=" and o.refferal_credited='$rebate_status'";	
	}
   
} 
$sql = "SELECT count(o.id) as total_count  from order_list
 as o inner join users as u on u.id=o.user_id inner join users as m on m.id=o.merchant_id Where  u.user_roles = 1   and total_refferal_amount>0 $filter";
...
$left_rec = $rec_count - ($page * $rec_limit);
  $query="SELECT m.id as merchant_id,m.name as merchant_name,o.*,u.name,u.id as u_id,u.otp_verified as user_verified,u.balance_usd,u.balance_inr,u.balance_myr,
	r.id as reffer_by_id,r.name as reffer_user_name,r.balance_inr as reffer_user_balance,r.mobile_number as reffer_user_mobile
	from order_list
 as o inner join users as u on u.id=o.user_id inner join users as m on m.id=o.merchant_id  inner join users as r on r.id=o.order_refer_id Where  u.user_roles = 1  and total_refferal_amount>0 $filter ORDER BY o.id DESC LIMIT $offset, $rec_limit";
    // die;      
        ...
        <div class="content-wrapper">
            ...
            <main class="main-wrapper clearfix" style="min-height: 522px;">
                <div class="container-fluid" id="main-content" style="padding-top:25px">
					<h2 class="text-center wallet_h">Refferal Rebate List</h2>
					<form class="sort row" method="post" action="">
					    ...
					    <div class="col-sm-12">
							...
							<label class="control-label col-sm-1" style="display:inline-block;">Refferal Rebate Status: </label>
				...
				<?php if($rec_count>25){ ?>        
					...
					<table class="table table-striped kType_table" id="kType_table">
						...
						<th>Action</th>
						   <th>Refferal By</th>
						   <th>Refferal Mobile Number</th>
						   <th>Refferal User koocash</th>
						   <th>Refferal Rebate Amount</th>
						 <th>Agent code</th>
						<th>Commissions</th>
						<th>Order Status</th>
                    	...
                    	while($row=mysqli_fetch_assoc($user)){  
							$r_status=$row['refferal_credited'];
							$user_id=$row['u_id'];
							
							$order_id = $row['id'];
							
							$user_mobile=$row['user_mobile'];
							$all_count = mysqli_fetch_assoc(mysqli_query($conn, "select count(id) as total_req from order_list where user_mobile='$user_mobile' and total_refferal_amount>0"))['total_req'];
							...
							$reffer_by_id=$row['reffer_by_id'];
							$merchant_id=$row['merchant_id'];
							if($reffer_by_id)
							{
								$ref_user_special_coin = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM special_coin_wallet WHERE user_id='$reffer_by_id' and merchant_id='6419'"));
								$ref_user_special_coin=$ref_user_special_coin['coin_balance'];
							
							}
							else
							{
								$ref_user_special_coin=0;
							}
							 
							 ?>
                        	  <tr style="<?php  if($otp_pre_verified=="n"){ echo "color:red;";} ?>">
                        		 <td> <?php echo $i; ?> </td>
								...
								<i style="font-size:20px;" class="fa fa-check credit_amount" rebate="<?php echo $row['total_refferal_amount'];?>" user_id="<?php echo $row['user_id']?>" 
								order_id="<?php echo $row['id']?>" wallet="<?php echo $row['wallet']?>" user_mobile="<?php echo $row['user_mobile']?>" reffer_by_id="<?php echo $row['reffer_by_id']; ?>" merchant_id="<?php echo $row['merchant_id']?>"></i>
								<?php } ?>
								<?php if($order_status && $r_status=="y"){  ?>
								
							<i  style="font-size:20px;" class="fa fa-remove undo_confirm_amount" rebate="<?php echo $row['total_refferal_amount'];?>" user_id="<?php echo $row['user_id']?>" 
								order_id="<?php echo $row['id']?>" wallet="<?php echo $row['wallet']?>" user_mobile="<?php echo $row['user_mobile']?>" merchant_id="<?php echo $row['merchant_id']?>"></i>
							
								<?php } ?>   
								</td>   
								<td><?php echo $row['reffer_user_name'];  ?></td>
								<td><?php echo $row['reffer_user_mobile'];  ?></td>  
								<td><?php echo number_format($ref_user_special_coin,2);  ?></td>
								<td><?php echo number_format($row['total_refferal_amount'],2);  ?></td>
            ...
            $('#kType_table').DataTable({
				...
				$(document).on('click', '.credit_amount', function(e){
				  ...
				  e.preventDefault();
					 ...
					 var reffer_by_id=$(this).attr('reffer_by_id');
					 ...
					 var user_id=$('#user_id').val();
					  ...
					  // alert(user_id);
					  // alert(order_id);
					 var data = {reffer_by_id:reffer_by_id,merchant_id:merchant_id,order_id:order_id,user_id:user_id,method:"walletcredit",rebate:rebate,w_type:wallet,order_rebate:'y'};
						$.ajax({
				...
				$(document).on('click', '.undo_confirm_amount', function(e){
				  ...
				  e.preventDefault();
					 ...
					 var user_mobile=$(this).attr('user_mobile');
					  ...
					  // alert(user_id);
					  // alert(order_id);
					 var data = {merchant_id:merchant_id,order_id:order_id,user_id:user_id,method:"walletdeduct",rebate:rebate,w_type:wallet,order_rebate:"y"};
						$.ajax({
							url :'walletcredit.php',
						  type:'POST',
						  dataType : 'json',
