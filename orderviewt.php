                ...
                <div class="well">
						  ...
						  <form id="criteria_form">
							...
							<div class="row">
								...
								<div class="col-sm-1" style="margin-top:20px">
									<button type="button" class="btn btn-danger" onclick="window.location.href='./orderviewt.php'"><?php echo $language['clear_criteria'];?></button>
							  </div>
							</div>
						  </form>
						</div!-->
                    ...
                    <table class="table table-striped">
                        ...
                        <tr>
							...
							<td style="min-width:250px;">Write Up</td>
                       ...
                       // $user_name = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id ='".$row['user_id']."'"));
                        ...
                        foreach ($amount_val as $key => $value){
						 ...
						 if($sstper>0){
							...
							$total_bill=($g_total+$row['order_extra_charge']+$row['deliver_tax_amount'])-($row['wallet_paid_amount']+$row['membership_discount']);
							$pro_str='';
							$q_str='';
							$r_str='';
							$show_remark='';
                        ...
                        <tr id="<?php echo $row['invoice_no']; ?>" class="<?php echo $todayorder; ?> fdfd <?php echo $callss; ?>" data-id="<?php echo $row['id']; ?>">
							...
							<td class="write_up" order_id='<?php echo $row['id']; ?>'><i class="fas fa-copy"></i></td>   
							...
							<td><?php  echo @number_format(($g_total+$row['order_extra_charge']+$row['deliver_tax_amount'])-($row['membership_discount']+$row['coupon_discount']),2); ?></td>
							
							<td><?php  echo @number_format($row['wallet_paid_amount'],2); ?></td>
							<td><?php echo @number_format(($g_total+$row['order_extra_charge']+$row['deliver_tax_amount'])-($row['wallet_paid_amount']+$row['membership_discount']), 2); ?></td>   
...
//}else{        
	   ...
	   var s_token=generatetokenno(16);
			var r_url="https://www.koofamilies.com/orderviewt.php?vs="+s_token;
    ...
    $(document).ready(function(){
	 ...
	 $(".s_order_detail").click(function(e){
		...
		$("#apply_criteria").on("click", function(e){
			...
			window.location.href = "./orderviewt.php?cr=" + criteria + "&q=" + query;
		  });
