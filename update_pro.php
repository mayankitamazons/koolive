...
if(!$_POST['always_active']){
 ...
 if($category)
 {
	$categories = mysqli_query($conn, "SELECT id FROM category WHERE status='0' and user_id ='".$loginidset."'and category_name ='".$category."'");
	$categoryrow=mysqli_fetch_assoc($categories);
	$category_id=$categoryrow['id'];
 }
 	$image_pic =  $_FILES["image_pic"]["name"]; 
    $image_code =  $_FILES["image_code"]["name"]; 
	$pro_data=mysqli_fetch_assoc(mysqli_query($conn,"select * from products where id='$id'"));
	// print_r($pro_data);
	// die;
if($_FILES['image_pic']['name'])
{
...
{
	$uniquesavename=$pro_data['image'];
	
}
$old_image=$pro_data['image'];
 if($image_pic=='')
	 $image_pic=$pro_data['image'];
 if($image_code=='')
...
//~ mysqli_query($conn,"UPDATE `products` SET `product_name`='ytyt',`category`='yty',`product_type`='tyt',`image`='yyytytyh.png' WHERE `id` = 4");
    ...
    if(isset($_FILES["image_pic"]) && $_FILES["image_pic"]["error"] == 0){
        ...
        if(($_FILES['image_pic']['size'] >= $maxsize) || ($_FILES["image_pic"]["size"] == 0)) {
             echo 'File too large. File must be less than 1 megabytes. '.$_FILES["image_pic"]["name"];
        }
        else
        {
			if(in_array($filetype, $allowed)){
				$destFile = "product/".$uniquesavename;
    ...
    if(isset($_FILES["image_code"]) && $_FILES["image_code"]["error"] == 0){
        ...
        // Verify MYME type of the file
        // Verify MYME type of the file
        if(($_FILES['image_code']['size'] >= $maxsize) || ($_FILES["image_code"]["size"] == 0)) {
            echo 'File too large. File must be less than 1 megabytes. '.$_FILES["image_code"]["name"];
        }
        else
        {
            if(in_array($filetype, $allowed)){
