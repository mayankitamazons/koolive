...
if($_POST['type'] == 'repeat_last_order'){
		...
		foreach ($product_ids as $key )
	{
		if(is_numeric($key))
		{
			...
			$cartData[$product['id']]['only_name'] = $product['product_name'];
			$cartData[$product['id']]['sub_varient'] = "<br>".$product_subitem;
			...
			$cart_html1 = "<input type='hidden' name='rebate_amount[]' class='rebate_amount' value=".$orderArray['rebate_amount']." id='".$product['id']."rebate_amount'><input type='hidden' name='rebate_per[]' value=".$rebate_amount[$i]." id='".$product['id']."rebate_per'><input type= hidden name='p_id[]' value=".$product['id']."><input type= hidden name='p_code[]' value= ".$prd_code."><input type='hidden' name='ingredients' value='".$product['remark']."'/><input type='hidden' name='varient_type[]' value=".$product['varient_type']."><input type='hidden' id='extra_child_".$product['id']."' name='extra' value='0.00'><input type='hidden' style='width:70px;' class='p_total' name='p_total[]' value= ".number_format($product_qty_price,2)." readonly  id='".$product['id']."_cat_total'><input style='width:70px;' type='hidden' name='p_price[]' value='".$amount_val[$i]."' readonly><input style='width:70px;text-align:right;' type='hidden' name='p_extra' id='".$product['id']."' value='0.00' readonly>";
			$subvarientsHtml = '';
			if($product_subitem != ''){
