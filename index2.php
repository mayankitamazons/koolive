...
if (empty($_GET['vs'])) {
	...
	if($_SESSION['locationsort'] || $_GET['locationsort'])
	$url = "index2.php?vs=" . md5(rand())."&locationsort=".$_SESSION['locationsort'];
	else
	$url = "index2.php?vs=" . md5(rand());
...
if($LocaSt){
$LOCSQL = "and users.city like '%$LocaSt%'" ;
}
if( isset($_GET{'page'} ) ) {
            $page = $_GET{'page'} + 1;
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <style>
                .locationbutton{ margin-top:2%;background-color: pink;padding: 13px;width: 100%;color: black;border-radius: 4px;font-size:20px }
				</style>
				<p> <button type="button" class="btn btn-danger" data-dismiss="modal" id="search_location"><?php echo $language['search_based_location']; ?></button>				</p>
                <form>
					<?php 
					$sql = mysqli_query($conn, "SELECT SQL_NO_CACHE CityName  FROM city WHERE status='1' GROUP BY CityName");
					$selected = '';
					while($data = mysqli_fetch_array($sql))
					{
					?>
					<p><a class="showLoader6" data-dismiss="modal" href="" onclick="window.location.assign('https://www.koofamilies.com/index2.php?locationsort=<?php echo $data['CityName'] ?>') ; return false;"><button class="locationbutton btn btn-primary"><?php echo $data['CityName'] ?></button></a></p>
					<?php	
					}	
					?>
...
<body>
	...
	if (isset($_GET['code']) && isset($_GET['id']) && is_numeric($_GET['id'])) {
		...
		if (!isset($_GET['apiusername'])) {
			...
			$user_row2 = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SQL_NO_CACHE * FROM users WHERE  id='$user_id'"));
	...
	<header class="header clearfix element_to_stick">
		...
		<div class="container">
			...
			<div id="logo">
				...
				<a href="index2.php?vs=<?php echo md5(rand()); ?>">
			...
			<nav class="main-menu">
				...
				<div id="header_menu">
					...
					<a href="index2.php?vs=<?php echo md5(rand()); ?>">KooFamilies</a>
				...
				<ul>
					...
					<?php } else { ?>
						...
						<li class="submenu"><a href="orderlist.php" class="show-submenu" style="font-size:16px"><?php echo $language['past_order']; ?></a></li>
						<li class="submenu"><a href="favorite.php" class="show-submenu" style="font-size:16px"><?php echo $language['my_fav']; ?></a></li>
						<li class="submenu"><a href="#"  type="normal_logout" class="logout"><?php echo $language['log_out']; ?>  </a></li>
					...
					<li class="submenu">
						...
						<ul class="">
							...
							<li><a href="index2.php?vs=<?php echo md5(rand()); ?>&language=english">English</a></li>

							<li><a href="index2.php?vs=<?php echo md5(rand()); ?>&language=chinese">Chinese</a></li>

							<li><a href="index2.php?vs=<?php echo md5(rand()); ?>&language=malaysian">Malay</a></li>
	...
	<main>
			...
			<div class="opacity-mask" data-opacity-mask="rgba(0, 0, 0, 0.6)">
				...
				<div class="container">
							...
							<form method="post" id="merchant_submit_form" action="#">
								...
								<div class="row no-gutters custom-search-input">
									...
									<div class="col-lg-6">
										<div class="form-group">
											...
											<select class="merchant_select form-control" name="merchant_select">
												...
												<?php
												$LocaSt = $_SESSION['locationsort'] ;
												$select =mysqli_query($conn,"SELECT SQL_NO_CACHE  name,id,mobile_number FROM users WHERE name LIKE isLocked='0' and user_roles=2 and city like '%$LocaSt%'");
												if(isset($_SESSION['locationsort'])){
												$select =mysqli_query($conn,"SELECT SQL_NO_CACHE  name,id,mobile_number FROM users WHERE name LIKE isLocked='0' and city like '%$LocaSt%' and user_roles=2");	
												}
									...
									<div class="col-lg-2">
										...
										<input type="submit" id="merchant_select_form" name="merchant_select_form" value="<?php echo $language['search']; ?>" style="margin-top: 0px;background:#e75480;">
					...
					<div style="margin-top:20px;">
						...
						<a href="#nearby_shops_section"><div type="button" id="search_location" style="margin-top:2%;background-color: #589442;padding: 13px;width: 100%;color: white;border-radius: 4px;" class="btn btn-primary col-xl-9 col-lg-10 col-md-8"><?php echo $language['search_by_location']; ?></div></a>

						<!--a href="jobs.php?vs=<?= md5(rand()) ?>" style="color:white;">

							<button type="button" id="" style="margin-top:1%;background-color: #589442;padding: 13px;width: 100%;color: white;border-radius: 4px;" class="btn btn-primary">



								<?php echo $language['post_job_list']; ?></button></a!-->
					...
					<div class="container" style="margin-top: 27px;">
					 ...
					 from classification_arrange_system as cs inner join users on users.id=cs.merchant_id LEFT JOIN about on  users.id=about.userid left join timings on  users.id=timings.merchant_id where cs.classfication_id='3' and users.user_roles = 2  $LOCSQL and users.shop_open=1 group by users.id
						...
						?>
							...
							<div class="main_title">
								...
								<div class="row">
									...
									<div class="col-md-8">
										<h2 style="text-align: left;color: white;"><?php echo $language['popular_restaurants']; ?></h2>
									</div>
		...
		while ($l_r = mysqli_fetch_assoc($l_query)) {
			...
			$sql = "select SQL_NO_CACHE users.banner_image,users.name, users.address,service.short_name,about.image,users.mobile_number,timings.*,users.order_extra_charge,users.delivery_plan,
			  ...
			  and users.user_roles = 2 $LOCSQL and users.shop_open=1 group by users.id ORDER BY cs.shift_pos ASC limit 20 ";
				...
				<div class="container <?php echo "classi_" . $l_r['id']; ?>" style="margin-top:5px;">
					...
					<div class="main_title">
						...
						<div class="row">
							...
							<div class="col-md-8">
								<h2 style="text-align: left;"><?php echo $classification_name; ?></h2>
							</div>
													...
													<div class="item_title">
														...
														<?php if ($rd[12] || $rd[13]) {
															if ($rd[13]) {
																...
																$d_str = "MYR " . number_format($rd[12], 2);
															}
														} else {
															$d_str = "Free Delivery";
														} ?>
		...
		<?php
			...
			users.working_text,users.working_text_chiness,users.banner_image,users.not_working_text,users.not_working_text_chiness FROM users 
			...
			WHERE users.user_roles = 2   $LOCSQL and users.isLocked= 0 group by users.id order by users.name asc";
			
		//echo $sql ;  start_time <  DATE_FORMAT(NOW(), '%h:%i') AND end_time > DATE_FORMAT(NOW(), '%h:%i') and // 
		//echo DATE_FORMAT(NOW(), '%h:%i') ;
		$result = mysqli_query($conn, $sql);

		$count = mysqli_num_rows($result);
		$number_of_page = ceil ($count / $results_per_page);  
		   $query = "$sql LIMIT ".$offset.",100";
		$result = mysqli_query($conn, $query);  	
		 $condition =100;
		   $rd = mysqli_fetch_all($result);  
		
		?>
		<div class="container margin_60_40 search_location_div search_location" id="nearby_shops_section">
				...
				<div class="col-12">
					...
					<div class="main_title version_2">
						...
						<div class="row">
							...
							<div class="col-md-8">
								<h2><?php echo $language['near_by_shop_2']; ?> (<?php echo $count ?>)</h2>
							</div>
					...
					<div class="list_home">
                        ...
                        for($i=0;$i<50;$i++){
						   ...
						   $work_str='';
								...
								{
									$work_str.="</br>".$language['sorry_rest']." ".$rd[$i][13]." (".$rd[$i][16].")";
								}
								else if($rd[$i][12])
								{
									$work_str.="</br>".$language['sorry_rest']." ".$rd[$i][12]." (".$rd[$i][15].")";
								}
							 // $distance=$rd[$i][11]['distance'];
                        ?>  <li class="showLoader6">
								<a href="view_merchant.php?sid=<?php echo $rd[$i][4];?>">
									<figure class="<?php if($working=="n"){echo "shop_close";} ?>">
									<?php if($rd[$i][14]) {  ?>

													<img ref="banner_image" data-src="<?php echo $image_cdn; ?>banner_image/<?php echo $rd[$i][14] ?>?w=400" class="owl-lazy lazy2 Sirv" alt="">
													...
													if ($rd[$i][3] == "") { ?>
														...
														<img src="images/logo_new.jpg" data-src="images/logo_new.jpg" class="owl-lazy" alt=""> <?php
																																			...
																																			} else { ?> <img data-src="<?php echo $image_cdn; ?>about_images/<?php echo $rd[$i][3] ?>?w=200" class="owl-lazy lazy2 Sirv" alt=""> <?php }
																																						} ?>
								...
								<?php if($sort_by!="sort_name"){ ?>
									...
									<!--small><?php echo $rd[$i][2]?></small!-->
									 <small style='color:red;'><?php if($working=="n"){if($work_str==""){
...
<div class="pagination_fg m-2">
		...
		if($page == $_GET['page']){
			echo '<a id="pag_prev" style="background-color: #ccc;color:red">' . $page . ' </a>'; 
		}else{
    echo '<a id="pag_prev"  style="background-color: #ccc;" href = "index2.php?page=' . $page . '&vs=' . md5(rand()) . '#nearby_shops_section">' . $page . ' </a>';   
		}
		}
	?>
	</div>
...
window.onload = init;
	...
	<script>
		...
		$(document).ready(function() {
			...
			var r_url = "https://koofamilies.com/index2.php?vs=" + s_token;
			...
			if ("geolocation" in navigator) { //check geolocation available 
				...
				navigator.geolocation.getCurrentPosition(function(position) {
					...
					var page_count="<?php echo $_GET['page']; ?>";
		...
		$('#loadnearbyshop').click('change', function() {
				$.ajax({
					type: "POST",
					url: "loadnearbyshop.php",
					data: {
					end: 50,
					city: 'city' ,
					},
