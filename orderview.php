                ...
                <div class="well">
                    ...
                    <table class="table table-striped">
                        ...
                        <tr>
							...
							<th><?php echo $language['container']; ?></th>
                            ...
                            <th><?php echo $language["chat"];?></th>
							 ...
							 <th><?php echo $language["amount"];?></th>
							  ...
							  <th><?php echo $language["pickup_type"];?></th>
                            <th><?php echo $language["total"];?></th>
							<?php if($sstper>0){ ?>
							 <th><?php echo "Service Fee ".$sstper." %"?></th>
							
							 
							 <!--th><?php echo $language['grand_total']."(Inc ".$sstper." % Service Fee)";?></th!-->
							 <th><?php echo $language['grand_total'];?></th>
							<?php } ?>
							...
							<th class="product_name test_product" style="min-width:150px;"><?php echo $language["remark"];?></th>
                       ...
                       // $user_name = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id ='".$row['user_id']."'"));
                        ...
                        foreach ($amount_val as $key => $value){
						 ...
						 if($sstper>0){
							...
							// $write_up_msg="order from Merchant name & address , telephone  to Customer name& deliceved address, telephone number cash term  total + delivery charges = grand total";
							// $total_bill=$g_total+$row['order_extra_charge'];
					$total_bill=($g_total+$row['order_extra_charge']+$row['deliver_tax_amount']+$row['special_delivery_amount'])-($row['wallet_paid_amount']+$row['membership_discount']+$row['coupon_discount']);
							...
							$c=0;
							 foreach ($product_ids as $key )
                                {
                                    ...
                                    if(is_numeric($key))
                                    {
                                        ...
                                        $pro_str.=$product['product_name'].",qty-".$quantity_ids[$c]."<br>";
                                    }
                                    else
                                    {
                                        $pro_str.=$key.'<br>';
								...
								if($row['remark_extra'])
								{
									$o_remark="Order Remark:".$row['remark_extra'];
								}
								if($row['plastic_box'])
								{
									$c_remark="</br>Container:".$row['plastic_box'];
								}
							 ...
							 $wallet=$row['wallet'];
							  // $write_up_msg="Order from ".$merchant_name['name'].",</br>".$merchant_name['google_map']." ,</br> Mobile - ".$merchant_name['mobile_number']." <br/>  To: ".$row['user_name']." </br>,".$row['user_mobile']." </br>".$row['location']."</br> Order Detail:". $pro_str." ".$o_remark.$c_remark."<br/>Cash term ".$g_total."+".$row['order_extra_charge']."=".number_format($total_bill,2);
							  if($v_comisssion)
								$write_up_msg="Order from </br>".$o_remark.$c_remark."<br/>Cash term Pay:".$total."-".$v_comisssion."(com)=".number_format($total-$v_comisssion,2)."</br>Receipt(".$wallet."):{".$total."+".$incsst."(SST)+".$row['order_extra_charge']."+".$row['deliver_tax_amount'].")-(".$row['wallet_paid_amount']."(WALLET)-".$row['membership_discount']."}=".number_format($total_bill,2)."</br>Pickup Type:".$row['pickup_type']."</br>".$merchant_name['name'].",</br>".$merchant_name['google_map']." ,</br> Mobile - ".$merchant_name['mobile_number']." <br/>  To: ".$row['user_name']." </br>,".$row['user_mobile'].$otp_str." </br>".$row['location']."</br> Order Detail:". $pro_str." ";
							else 
								$write_up_msg="Order from </br>".$o_remark.$c_remark."<br/>Cash term Pay:".number_format($total,2)."</br>Receipt(".$wallet."):{".$total."+".$incsst."(SST)+".$row['order_extra_charge']."+".$row['deliver_tax_amount'].")-(".$row['wallet_paid_amount']."(WALLET)-".$row['membership_discount']."}=".number_format($total_bill,2)."</br>Pickup Type:".$row['pickup_type']."</br>".$merchant_name['name'].",</br>".$merchant_name['google_map']." ,</br> Mobile - ".$merchant_name['mobile_number']." <br/>  To: ".$row['user_name']." </br>,".$row['user_mobile'].$otp_str." </br>".$row['location']."</br> Order Detail:". $pro_str." ";
						
						// $write_up_msg="Order from </br>".$o_remark.$c_remark."<br/>Cash term Pay:".number_format($total,2)."</br>Receipt(".$wallet."):".$total."+".$incsst."(SST)+".$row['order_extra_charge']."+".$row['deliver_tax_amount']."=".number_format($total_bill,2)."</br>Pickup Type:".$row['pickup_type']."</br>".$merchant_name['name'].",</br>".$merchant_name['google_map']." ,</br> Mobile - ".$merchant_name['mobile_number']." <br/>  To: ".$row['user_name']." </br>,".$row['user_mobile'].$otp_str." </br>".$row['location']."</br> Order Detail:". $pro_str." ";
                        ...
                        <tr id="<?php echo $row['invoice_no']; ?>" class="<?php echo $todayorder; ?> fdfd <?php echo $callss; ?>" data-id="<?php echo $row['id']; ?>">
							...
							<?php
                                ...
                                ?>
								<input type="button" next_status="<?php echo $n_status; ?>" style="background-color:<?php echo $s_color;?>;<?php echo $b_color;?>" class= "status btn btn-primary" value="<?php  echo $sta;?>" status="<?php echo $row['status'];?>" data-invoce='<?php echo $row['invoice_no'];?>' data-id="<?php echo $row['id']; ?>"/>
								...
								<?php 
									...
									{  
										...
										if($v_array[$i])
										{
											...
											#echo count($v_array)."====".$product['varient_must']."===".$show_error;
											//$sub_product_varient_array[$key] = 'no';
										}
									...
									echo '<br/>';
										...
										if($show_error == 'yes'){
											//echo '<br/><br/><input type="button" style="background-color:red" class= "btn btn-danger" value="Error in order" />';
										}else if($allproduct_price_digit == $cash_term_payment_digit){
											//echo 'same';
										}else{
											//echo 'not same';
											//echo '<br/><br/><input type="button" style="background-color:red" class= "btn btn-danger" value="Error in order" />';
	...
	select.rider_info_select{
		...
		border-color:red;
						...
						<?php }?>
							...
							<!--<td class="writeup_set" id="writeup_set_<?php  echo $row['id'];?>" order_id='<?php echo $row['id']; ?>'><i class="fa fa-copy" style="font-size:25px;margin-left: 10%;"></i>
							</td>-->
							<td>
							<a style="display:none;color: #fff;background-color: #337ab7;border-color: #2e6da4;" class="btn btn-primary copy-text cust_<?php echo $row['id'];?>" onclick="copyToCustAdd('#writeup_set_<?php  echo $row['id'];?>','cust_<?php echo $row['id'];?>','<?php echo $row['id'];?>')"> Copy </a>
			
							<a style="display:none;color: #fff;background-color: #337ab7;border-color: #2e6da4;" class="btn btn-primary  copy-text orderdeatils_<?php echo $row['id'];?>" onclick="copyToOrderDetails('#od_copy_details_<?php  echo $row['id'];?>','orderdeatils_<?php echo $row['id'];?>')"> Copy Order Details </a>
							<br/>
							<span class="writeup_set" id="writeup_set_<?php  echo $row['id'];?>" order_id='<?php echo $row['id']; ?>'><i class="fa fa-copy" style="font-size:25px;margin-left: 10%;"></i><span>
							</td>
		
		
								 <td><?php echo $row['remark_extra']; ?></td>
							   <td class="table_number_<?php echo $row['id']?>"><?php echo $row['section_name'];?></td>
							   <td><?php echo $row['plastic_box']; ?></td>
                            ...
                            <td>
							<a href="https://api.whatsapp.com/send?phone=<?php  echo $row['user_mobile']?>" target="_blank">
							
							<img src="images/whatapp.png" style="max-width:40px;"/></a></td>
							...
							<td class="quantity_<?php echo $row['id'];?>"><?php
                                ...
                                ?></td>
							<td>
							<?php if($row['pickup_type']){
								echo $row['pickup_type'];
							} else { echo "Delivery";}								?>
							</td>
						...
						$v_str=$row['varient_type'];
							...
							<td  style="font-size:18px;"  order_id='<?php echo $row['id']; ?>'> 
								...
								if($row['reviewed']==1)
								{
								  ...
								  ?>
								  <span class="btn btn-yellow review_detail" order_id="<?php echo $row['id']; ?>" style="color:black;">Review</span>
							 ...
							 <!--td>
                                <?php
                                    $o_id = $row['id']; 
                                    $dt = mysqli_fetch_assoc(mysqli_query($conn, "SELECT agent_code FROM order_list WHERE id='$o_id'"));
                                    if($dt['agent_code'] == 0 || $dt['agent_code'] == null){
                                        echo " - ";
                                    }else{
                                        echo $dt['agent_code'];
                                    }
                            ...
                            <td>
                                <?php 
                                    ...
                                    $comm = mysqli_fetch_assoc(mysqli_query($conn, "SELECT qty FROM agent_commissions WHERE order_id='$o_id'"))['qty'];
                                    echo ($dt['agent_code'] == 0 || $dt['agent_code'] == null) ? " - " : $comm;
                                ?>
                            </td!-->
                        </tr>    
                        <?php   $i++; }
		...
		<input type="text" id="paidkoocoin" value="0" class="amount" readonly name="paidkoocoin" style="background-color:#6dafe2; font-size: 20px; width: 30%;margin-left: 30%;border: 1px solid #555555 ;padding-left: 4px;">
                                        ...
                                        <div class="col-sm-7" style="float: left;">
                                           ...
                                           <input type="hidden" name="merchant_ses" id="merchant_ses" value="<?php echo $loginidset;?>">
                                            ...
                                            <div class="modal-footer" style="padding-bottom:2px; border-top: none;padding: 0px;padding-top: 5px;">
                                               ...
                                               <input type="submit" style="width:200px;height:50px;background-color: #99e1dc;" value="<?php echo $language['print']; ?>" onclick="this.form.target='_blank';return true;">
                                        ...
                                        <div class="modal-body" style="padding-bottom:0px;">
                                            <div class="col-sm-12">
												...
												<?php
													...
													$sql = "select * from order_list where status in(0,2) and merchant_id='$Merchantid' group by invoice_no limit 0,100";
						 ...
						 <div class="modal fade" id="logout_model" role="dialog" style="margin-top:15%;">
                    ...
                    <div class="modal-body" style="padding-bottom:0px;">
                        ...
                        <div class="col-md-12" style="text-align: center;">
                          ...
                          <h5 id="shop_model_text">
							Are you sure that you want to close shop?
						...
						<div class="row">
						  ...
						  <div class="col-md-6">
						   <button  type="button" class="btn btn-primary logout" t_type="shop_close" data-dismiss="modal" style="margin-left:19%;width:50%;margin-bottom: 3%;text-align: center;color:black"><?php echo $language['yes']; ?></button>
								  ...
								  <div class="col-md-6">
<button  type="button" class="btn btn-primary" id="no_logout_button"  style="margin-left:19%;width:50%;margin-bottom: 3%;text-align: center;color:black;"><a href="index.php"  style="color:black;"><?php echo $language['no']; ?></a></button>
						...
						<div class="modal fade" id="reviewdetailmodel" role="dialog">						
							<div class="modal-dialog">
							<!-- Modal content-->		
							<div class="modal-content">	
							<div class="modal-header">	
							...
							<div class="modal-body" style="padding-bottom:0px;">
							<div class="col-sm-10" id="reviewdisplaydetailmodel_load">		  				
			  ...
			  <div class="col-md-6 input-has-value">
							...
							<div style="padding-top: 5px;display: flex;font-weight:bold;">
                                            ...
                                            <div style="padding-top: 5px;font-weight:bold;display: flex;" class="input-has-value">
                                                <span style="font-size:20px; width: 40%;    border: 1px solid; border-bottom-left-radius: 2px; border-top-left-radius: 2px;"><?php echo $language['paid']; ?></span>
                                            ...
                                            <div style="padding-top: 5px;font-weight:bold;display: flex;">
                                                <span style="font-size:20px; width: 40%;    border: 1px solid;border-bottom-left-radius: 2px; border-top-left-radius: 2px;"><?php echo $language['change']; ?></span>
											...
											<br>
											<input type="submit" class="btn btn-primary btn-md waves-effect waves-light <?php if($process_password_need){ echo "process_password_need";}?>" value="<?php echo $language['submit']; ?>">
...
<div id="WalletModel" class="modal fade" role="dialog">
				  			<div class="modal-dialog">
							    ...
							    <div class="modal-content">
					      			...
					      			<div class="modal-header">
					        			<h2> <?php echo $language['total_amount_rm']; ?><span id="total_wallet_amount"></span></h2>  
										  <button type="button" class="close"  data-dismiss="modal">&times;</button>
											...
											<div>
											  <div class="input-group mb-2" style="margin-bottom:0px !important;">
												...
												<input type="number" autocomplete="tel" maxlength='10' id="mobile_number"   class="mobile_number form-control" style="min-width:250px;"  placeholder="123115670" name="mobile_number" required="" />
												 <input type="submit" mode="confirm" id="confirm_payment" class="btn btn-primary" value="<?php echo $language['confirm']; ?>"/>
											</div>
...
$(document).ready(function(){
	...
	$( "#DoubleModel" ).prop( "disabled", true );
    var test1 = {'Token': Math.random()};
    localStorage.setItem('TestData',JSON.stringify(test1));
    var test = localStorage.getItem("TestData");
});
</script>      
<div class="modal" id="DoubleModel" style="max-width:80%;">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
              Koofamilies is open in another window. Click "User Here" to use Koofamilies in this window.   
      </div>
      <div class="modal-footer">
        <a href="javascript:void(0);" id="userHere" class="btn btn-success">User Here</a>
      </div>
    </div>
  </div>
</div!-->      
...
//}else{        
	   ...
	   var s_token=generatetokenno(16);
			var r_url="https://www.koofamilies.com/orderview.php?vs="+s_token;
...
//end code
	...
	*/
	...
	$('#no_logout_button').click(function() {
		$('#logout_model').modal('hide');
	});
		$('.writeup_label').click(function() {
			// alert(3);
			var copyText =$(this).attr('l_text');
	...
	$(".review_detail").click(function(e){
      var s_id = $(this).attr('order_id');
		$("#order_product_name").html("order id "+s_id);
		$.ajax({
                type: "POST",
                url: "getreview.php",
                data: {s_id:s_id},
                success: function(data) {
                  ...
                  $('#reviewdisplaydetailmodel_load').html(data);
	...
	if(status!=4 || status!==5 || status!='')
	{
         $(this).attr("disabled", "disabled");  
			...
			{
				var update_able_status=$(this).attr('next_status');
				// if(update_able_status=='' || update_able_status=="undefined")
				// alert(update_able_status);
				if(update_able_status!='')
				{
					 $.ajax({
						url : 'update_status.php',
						type: 'POST',
						data: {id:data_id, status: update_able_status},
						success:function(data){
							//~ alert(1);
							// location.reload();
							var s_token=generatetokenno(16);
