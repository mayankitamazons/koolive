...
function toggle(n) {
	...
	for(var i=0;i<menus.length;i++){
        if((i == (n-1)) && (menus[i].style.display != "block")){
			...
			element[i].classList.add("active_city");
        }else{
            menus[i].style.display = "none";
			element[i].classList.remove("active_city");
		}
    } 
};
</script>
<div id="myModal" class="modal fade" style="">
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			<ul style="list-style-type: none;" class="list-group p-2 text-center">
			<?php
			$sql_fstate = mysqli_query($conn, "SELECT StateName FROM city GROUP BY StateName order by StateName asc");
			while($data_fstate = mysqli_fetch_array($sql_fstate))
			{if($_SESSION["statesort"] == $data_fstate['StateName']){}
			?>
				<li class="list-group-item"><p class="city_pp"><?php echo $data_fstate['StateName'];?></p></li>
			<?php }?>
			<?php 
 			...
 			<li class="list-group-item submenuparent" id="submenu_<?php echo $ck;?>"><p class="city_pp <?php echo strtolower($data['StateName']);?> aa_city" style="<?php echo $ss_style;?>"><a href="javascript:void(0);" link="https://koofamilies.com/index_newcitypopup.php?vs=<?php echo md5(rand()); ?>&locationsort=<?php echo $data['CityName'];?>&statesort=<?php echo $data['StateName']; ?>&language=<?php echo $_SESSION["langfile"];?>" class="locationbutton locationNewbutton"><?php echo $temp_city_name;//$data['CityName'] ?> (<?php echo $count_shops1['shop_count'];?>  <?php echo $language['shops'];?>)</a> 
			<a href="#" onclick="toggle(<?php echo $ck;?>);" >
				...
				while($data_location = mysqli_fetch_array($sql_location)){?>
					<li class="list-group-item" style="font-weight:normal" link="https://koofamilies.com/index_newcitypopup.php?vs=<?php echo md5(rand()); ?>&locationsort=<?php echo $data['CityName'];?>&statesort=<?php echo $data['StateName']; ?>&language=<?php echo $_SESSION["langfile"];?>" class="locationbutton locationSubNewbutton"><?php echo $data_location['l_name'];?></li>
				<?php }?>
				</ul>
			...
			<p class="dont_see_location text-center">Don't see your locaion here?</p>
			<!--- END NEW CODE---->
...
<div class="modal fade" id="dontseeLocation" tabindex="-1" role="dialog" aria-labelledby="dontseeLocation" aria-hidden="true">
  <div class="modal-dialog  modal-sm modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>Let us know where we should launch next and we'll keep you updated!!</p>
		<form>
          <div class="form-group">
		    <label for="recipient-name" class="col-form-label">Your Area:</label>
              <input list="locationsLists" name="locationsList" id="locationsList">
		  ...
		  <p style="display:none" class="contact_admin_location text-danger"> Sorry ! We can't able to find you location. Please contact the <a class="" href="">support team</a><p>
		  
        </form>
      </div>
      <div class="pb-2 text-center">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" id="submit_user_area">Submit</button>
