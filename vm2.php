...
$categories_q = mysqli_query($conn, "SELECT SQL_NO_CACHE  * FROM cat_mater WHERE UserID ='".$id."' and IsEnable=1 ");
if($product['pro_ct'] > 0) { ?>
    <div class="col-md-12 merchant-layout-2">
        <div class="filter-button-group parent-category-menu">
            ...
            $categories = explode(",",$category_a['CatName']);
			 $query="select SQL_NO_CACHE id from arrange_system WHERE user_id ='".$id."' and page_type='c' and status='active' group  by entity_id";
			
			$orderquery=mysqli_query($conn,$query);
			$ordercount=mysqli_num_rows($orderquery);
		     $query="select SQL_NO_CACHE id from arrange_system WHERE user_id ='".$id."' and page_type='p' and status='active' group  by entity_id";
            ...
            <div class="col-4 col-sm-3">
                <div class="sub_category_grid">
                    ...
                    foreach ($categories as $category)
                    {
						if($ordercount>0)
						{
							 $q="SELECT SQL_CACHE category.*,arrange_system.shift_pos FROM category inner join arrange_system on  category.id=arrange_system.entity_id and arrange_system.user_id='".$id."'
            ...
            <div class="col-8 col-sm-9 pl-2">
                <div class="grid">
					...
					if($pcount>0)
					{
						 ...
						 $q="SELECT SQL_NO_CACHE products.*,arrange_system.shift_pos FROM products inner join arrange_system on  products.id=arrange_system.entity_id 
                    ...
                    while ($row=mysqli_fetch_assoc($total_rows)){
                        ...
                        // $sale_total_rows = mysqli_query($conn,$saleQuery);
                        // while ($salerow=mysqli_fetch_assoc($sale_total_rows)){
                            // if($row['id'] == $salerow['valu']){
                                // $total_sale += $salerow['qty'];
                            // }    
                        // }
                         ...
                         //print_R($row);
						 //die;
                        // $commission = mysqli_fetch_assoc(mysqli_query($conn, "SELECT status FROM commission_status WHERE prod_id = '$row[id]' ORDER BY date DESC LIMIT 1"))['status'];
                                ...
                                <form action="#" method="post" class="set_calss" data-id="<?php echo $row['id'] ?>" data-code="<?php echo $row['product_type'] ?>"  data-pr="<?php echo $row['product_price'] ?>">
									...
									if(!empty($row['image'])) { ?>
                                        <div class="col-md-5 col-sm-5">
											...
											<?php if($product_zoom){ ?>
											    <?php
											        /*$variableA1=$row['product_name'];
											        $variableA2=$row['product_price'];*/
											        $variableA1=str_replace('"','',$row['product_name']);
											        $variableA2=$row['product_price'];
											    ?>
											    <a class="adminDemoVideo" href="https://koofamilies.sirv.com/product/<?php echo $row['image']; ?>" data-fancybox data-caption="<div class='card' style='text-align:center;'>
                                                    ...
                                                    <img data-src="https://koofamilies.sirv.com/product/<?php echo $row['image']; ?>" class="lazy" />
                                                </a>
											<?php } else { ?>
											    <!--img  src="<?php echo $site_url; ?>/images/product_images/<?php echo $row['image']; ?>" class="img-fluid lazy" !-->
											    <img  data-src="https://koofamilies.sirv.com/product/<?php echo $row['image']; ?>" class="img-fluid lazy" >
											<?php } ?>
                                                </div>
                                        </div>
                    ...
                    foreach ($products_id_global as $key => $product_id) {
                        $result = array();
                        $total_rows = mysqli_query($conn, "SELECT SQL_NO_CACHE * FROM sub_products WHERE product_id='".$product_id."'");
                        ...
                        if($ref_result){
                            ...
                            while ($row=mysqli_fetch_assoc($total_rows)){
								if($hike_per>0)
								{
								$new_sub_price_direct =(($hike_per / 100) * $row['product_price'])+ $row['product_price'];
								$new_sub_price=ceiling($new_sub_price_direct,0.05);
								}
								else
								{
									$new_sub_price=$row['product_price'];
								}  
								$new_sub_price=number_format($new_sub_price,2);
                                $item = array( "id" => $row['id'], "name" => $row['name'], "product_id" => $product_id,'product_price' => $new_sub_price);
