...
{
	$sid=$_GET['sid'];
	$url="completeorder.php?sid=".$sid."&ms=".md5(rand());
...
$infrm_where = '';
$rider_where = " where  cancel_order!=1 and rider_complete_order = 1 and created_on >'2021-04-07 12:00:00'";
...
".$rider_where." ".$infrm_where."
order by order_list.id desc limit 0,500";
//echo $query;
...
$order_inf_count = 0;
$pending_infrms =  mysqli_query($conn,'select count(*) as order_cnt from order_list inner join users as u on u.id=order_list.merchant_id inner join users as user on user.id=order_list.user_id where inform_rider_arrive_minute = 0 and cancel_order!=1 and created_on >"2021-04-21 12:00:00"');
...
$riders_processjob = mysqli_query($conn,"select r.r_id,count(od.id)as ordercount from tbl_riders as r LEFT JOIN order_list as od ON od.rider_info = r.r_id and rider_complete_order = 0  and cancel_order!=1  where r_status = 1 and r_online = 1 group by r.r_id");
$riderJobArray = array();
...
<div class="container" >
  <h2 style="text-align:center">Completed Order</h2>
...
<label style="background-color:green;border-color:green;cursor:Auto;margin:3px;" class="status btn btn-primary"><?php echo $gvalue['r_name']." (".$riderJobArray[$gvalue['r_id']].")";?></label>
<?php }?>

<?php foreach($red_riders as $rkey => $rvalue){?>
<label style="background-color:red;border-color:red;cursor:Auto;margin:3px;" class="status btn btn-primary"><?php echo $rvalue['r_name']." (".$riderJobArray[$rvalue['r_id']].")";?></label>
<?php }?>
</div>
...
Payment Proof </a>
					  ...
					  Merchant Order
							...
							<p style="width:200px"><?php echo $r['merchant_name']; ?></p>
		
		<b>Invoice:</b> # <?php echo $r['invoice_no']; ?>
		<br/>
...
<?php /*?>
	 ...
	 <?php }?>
	   ...
	   <p style="padding-bottom:10px;">
		Bank price: <input type="text" class="form-control admin_bank_price" name="admin_bank_price" id="admin_bank_price" value="<?php echo $row['admin_bank_price'];?>" order_id="<?php echo $row['id']; ?>" style="width:71px;float:right;height:10%;;background-color:white!important;" />
		</p>
		<p style="padding-bottom:10px;">
		Cash price: <input type="text" class="form-control admin_cash_price" name="admin_cash_price" id="admin_cash_price" value="<?php echo $admin_cash_price;?>" order_id="<?php echo $row['id']; ?>"   style="width:71px;float:right;height:10%;;background-color:white!important;"/>
		</p>
	    ...
	    <td>
		 <p style="padding-bottom:10px;">
	   Bank price: <input type="text" class="form-control rider_bank_amount" name="rider_bank_amount" id="rider_bank_amount" value="<?php echo $row['rider_bank_amount'];?>" order_id="<?php echo $row['id']; ?>"   style="width:71px;float:right;height:10%;;background-color:white!important;" />
		</p>
		 <p style="padding-bottom:10px;">
		Cash price: <input type="text" class="form-control rider_cash_amount" name="rider_cash_amount" id="rider_cash_amount" value="<?php echo $row['rider_cash_amount'];?>" order_id="<?php echo $row['id']; ?>" style="width:71px;float:right;height:10%;;background-color:white!important;"/>
		</p>
	   </td>
	   <td><p style="padding-bottom:10px">
		Commission: <input type="number" class="form-control admin_commission_price acp_<?php echo $row['id']; ?>" name="admin_commission_price" id="admin_commission_price" value="<?php echo $row['admin_commission_price'];?>" order_id="<?php echo $row['id']; ?>"  style="width:50%;float:right;height:10%;;background-color:white!important;" />	
...
/*End riders*/
	setInterval(function(){ 
		...
		var r_url="https://www.koofamilies.com/completeorder.php?ms="+s_token;
		...
		if(cnfrmDelete==true){
			$.ajax({
					...
					success:function(res){
						//console.log(res);
						//location.reload(true);
						var g_link = "https://docs.google.com/spreadsheets/d/1hDT0Qj5mHKR7_9VDjhCCpwe7-7jMablzr-vJ693-m6s/edit?usp=sharing";
