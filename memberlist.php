...
$defalut_plan = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM membership_plan WHERE  default_plan='y' and user_id='".$_SESSION['login']."'"));
$default_plan_id=$defalut_plan['id'];
...
// die;
    // $query="SELECT membership_plan.plan_name,membership.plan_id,u.* FROM users as u left join user_membership_plan as membership on u.id=membership.user_id and   membership.plan_active='y' inner join membership_plan on membership_plan.id=membership.plan_id WHERE u.referred_by= '".$referral_id."'";
    $query="SELECT membership_plan.plan_name,membership.plan_id,u.* FROM users as u left join user_membership_plan as membership on u.id=membership.user_id and membership.plan_active='y' 
left join membership_plan on membership_plan.id=membership.plan_id
WHERE u.referred_by= '".$referral_id."'";
if($referral_id)
...
<body class="header-light sidebar-dark sidebar-expand pace-done">
            ...
            <main class="main-wrapper clearfix" style="min-height: 522px;">
                <div class="container-fluid" id="main-content" style="padding-top:25px">
					<h3>Member List</h3>  
						...
						<h4 style="font-size:1.3em;">Your own referral ID: <?php echo isset($referral_id) ?  $referral_id : '';?> <a class="btn btn-primary" href="signup_referral.php?m_id=<?php echo $profile_data['id'];?>">Add Member</a><button class="btn btn-info copy-link" style="margin-left:10px;" onclick="copy_url()">Copy link</button></h4>
                    	...
                    	$i=1;
							...
							$isLocked=$row['isLocked'];
                        	 ...
                        	 <tr style="<?php  if($isLocked=="1"){ echo "color:red;";} ?>">
									...
									<td>
									<?php if($row['plan_name']){ echo $row['plan_name'];} else {
										$url="give_subscription.php?plan_id=".$default_plan_id."&mobile=".$row['mobile_number'];?>
									<a href="<?php echo $url;?>" class="btn btn-primary">Add Membership</a>
									<?php } ?>
									</td>
	...
	<script>
		...
		function copy_url(){
            ...
            var login_id = '<?php echo $_SESSION['login']; ?>';
            // referral_id = referral_id.replace(/ /g, '%20');
              document.getElementById("dummy_id").value="https://www.koofamilies.com/signup_referral.php?m_id="+login_id;
